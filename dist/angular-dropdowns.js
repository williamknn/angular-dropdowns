...
(function (window, angular) {
  ...
  dd.run(['$templateCache', function ($templateCache) {
    $templateCache.put('ngDropdowns/templates/dropdownSelect.html', [
      '<div ng-class="{\'disabled\': dropdownDisabled}" class="wrap-dd-select" tabindex="0" style="border-left: 3px solid {{ dropdownModel.color }}">',
      '<span class="selected"> <span class="icon"><img data-ng-src="/lib/routeasy/images/icons/flat/icon-{{ dropdownModel.value.vehicle.type || \'Question\' }}.png" /></span> <span class="selected-title">{{ (dropdownModel.value.vehicle.name || dropdownModel.value.vehicle.type) !== "Orphans" ? (dropdownModel.value.vehicle.name || dropdownModel.value.vehicle.type) : "Órfãos" }}</span> <span class="selected-desc">{{ dropdownModel[labelField]  || \'Selecione a rota\' }}</span> </span>',
      '<ul class="dropdown">',
    ...
    $templateCache.put('ngDropdowns/templates/dropdownSelectItem.html', [
      ...
      ' ng-click="selectItem()">',
      ' <span class="icon"><img data-ng-src="/lib/routeasy/images/icons/flat/icon-{{ dropdownSelectItem.value.vehicle.type }}.png"></span>',
      ' <span class="title">{{(dropdownSelectItem.value.vehicle.name || dropdownSelectItem.value.vehicle.type) !== "Orphans" ? (dropdownSelectItem.value.vehicle.name || dropdownSelectItem.value.vehicle.type) : "Órfãos"}}</span><br />',
      ' <span class="desc" ng-if="dropdownSelectItem.value._return_to_depot">{{dropdownSelectItem[dropdownItemLabel]}} - {{(dropdownSelectItem.value.points.length - 2) > 1 ? (dropdownSelectItem.value.points.length - 2) + " pedidos" : (dropdownSelectItem.value.points.length - 2) + " pedido"}}',
      ' {{dropdownSelectItem.value.distance ? " - " + dropdownSelectItem.value.distance + " km" : ""}}',
      ' {{dropdownSelectItem.value.duration ? " - " + dropdownSelectItem.value.duration.toFixed(1) + " h" : ""}}</span>',
      ' <span style="clear:both;"></span>',
      ' <span class="desc" ng-if="!dropdownSelectItem.value._return_to_depot">{{dropdownSelectItem[dropdownItemLabel]}} - {{(dropdownSelectItem.value.points.length - 1) > 1 ? (dropdownSelectItem.value.points.length - 1) + " pedidos" : (dropdownSelectItem.value.points.length - 2) + " pedido"}}',
      ' {{dropdownSelectItem.value.distance ? " - " + dropdownSelectItem.value.distance + " km" : ""}}',
      ' {{dropdownSelectItem.value.duration ? " - " + dropdownSelectItem.value.duration.toFixed(1) + " h" : ""}}</span>',
      ' <span style="clear:both;"></span>',
      '</a>',
